...
import com.stuintech.lacrimis.util.NeighborList;
...
public class AdvancedRuneBlock extends BasicRuneBlock {
    ...
    public static BlockPos getDuct(World world, BlockPos pos) {
        ...
        if(NeighborList.isEdge(i)) {
            for(int j = -1; j < 2; j++) {
                ...
                if(block instanceof DuctRuneBlock) {
                    if(duct) {
                        world.setBlockState(pos, world.getBlockState(pos).with(PIPE, 3));
                        return -3;
                    }
                    duct = true;

                    //Save duct location
                    world.setBlockState(pos, world.getBlockState(pos).with(PIPE, j + 1));
                }
            }
        }
        return tier;
